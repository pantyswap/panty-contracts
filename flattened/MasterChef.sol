...
// File: contracts\PantyToken.sol

pragma solidity 0.6.12;


// PantyToken with Governance.
contract PantyToken is BEP20('Panty Token', 'PANTY') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PANTY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PANTY::delegateBySig: invalid nonce");
        require(now <= expiry, "PANTY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PANTY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PANTYs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PANTY::_writeCheckpoint: block number exceeds 32 bits");
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe panty transfer function, just in case if rounding error causes pool to not have enough PANTYs.
    function safePantyTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 pantyBal = panty.balanceOf(address(this));
        if (_amount > pantyBal) {
            panty.transfer(_to, pantyBal);
        } else {
            panty.transfer(_to, _amount);
        }
    }
...
// MasterChef is the master of Panty. He can make Panty and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PANTY is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accPantyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPantyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // PANTY tokens created per block.
    uint256 public pantyPerBlock;
    // Bonus muliplier for early panty makers.
    ...
    // Stake PANTY tokens to MasterChef
    ...
    // Withdraw PANTY tokens from STAKING.
    ...
    // Safe Panty transfer function, just in case if rounding error causes pool to not have enough PANTYs.
    function safePantyTransfer(address _to, uint256 _amount) internal {
        syrup.safePantyTransfer(_to, _amount);
    }
    ...
    function updateEmissionRate(uint256 _pantyPerBlock) public onlyOwner {
        massUpdatePools();
        pantyPerBlock = _pantyPerBlock;
        emit UpdateEmissionRate(msg.sender, _pantyPerBlock);
    }
}
